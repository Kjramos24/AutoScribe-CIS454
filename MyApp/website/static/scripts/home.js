...
showSearchResult(resultList){
  ...
  resultList.forEach((result) => {
    ...
    const content = `
    <div class="source-card bg-white m-[10px] max-h-[150px] rounded-md p-[10px] flex">
                    <div class="sc-content">
                        <div class="sc-title-expand flex items-center">
                            <h2 class="font-bold text-2xl mr-[10px]">${resultList[i][0].replace(/'/g, "\\'")}</h2>
                        ...
                        <a class = "text-[#040cda] text-sm" href="#">${resultList[i][1].replace(/'/g, "\\'")}</a>
                        <p class = "w-[90%] text-xs mt-[10px] text-ellipsis">${resultList[i][2].replace(/'/g, "\\'")}</p>    
                    </div>
